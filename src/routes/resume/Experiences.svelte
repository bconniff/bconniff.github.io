  ...
  <Card title={title}>
    {#each experiences as experience}
      <Section
        ...
        <ul slot="header-right" class="list-['–_'] pl-4 py-2 leading-tight screen:sm:grow screen:sm:list-none screen:sm:pl-0 screen:sm:py-0 screen:sm:leading-normal">
          {#each experience.positions as position}
            <li class="screen:sm:text-right">
              <span class="print:text-sm">{position.name}</span>
              <span class="screen:text-indigo-800 dark:text-indigo-300 print:italic ml-2 text-sm">
                {position.startDate}
                {#if position.endDate}
        ...
        {#if experience.achievements.length}
          ...
          <ul class="list-['–_'] text-sm pl-4 leading-snug">
            {#each experience.achievements as achievement}
              <li class="my-1">{achievement}</li>
            {/each}
          </ul>
        {/if}
      </Section>
